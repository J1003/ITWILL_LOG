...
==================== */
CREATE TABLE TEST (
    NUM NUMBER(5, 2), -- 숫자타입이고, 전체 자리수는 5자리 : 정수부3자리, 소수부 2자리
    STR1 CHAR(10), -- 고정길이 문자열
    STR2 VARCHAR2(10), -- 가변길이방식으로 저장하는 문자열. variable
...
-- 오류발생 : 정수부 3자리 저장 가능한데, 4자리 저장 시도하면 오류발생.
...
/* *** NULL의 특성 ****
-- NULL : 데이터가 없는 상태
-- NULL 은 비교처리가 안 됨 : =, <>, !=, >, <, >=, <= 비교처리 의미 없음 - 결과 NULL
-- NULL 과의 연산결과는 항상 NULL(연산 의미 없음)이다.
-- NULL 값에 대한 조회(검색)는 IS NULL, IS NOT NULL 키워드로 처리
----------------------- */
-- IS NULL & IS NOT NULL
SELECT * FROM TEST;
SELECT * FROM TEST WHERE NUM = NULL; -- 조회 안 됨(비교연산 의미없음, 하지마라)
SELECT * FROM TEST WHERE NUM IS NULL; -- IS NULL 사용 검색할 것

SELECT * FROM TEST WHERE NUM <> NULL; -- 조회 안 됨(비교연산 의미없음, 하지마라)
SELECT * FROM TEST WHERE NUM IS NOT NULL; -- IS NOT NULL 사용해서 검색할 것
-----------------------------
...
SELECT * FROM TEST;
SELECT NUM, NUM + 100 FORM TEST;
SELECT NUM, NVL(NUM, 0), NVL(NUM, 0) + 100 FROM TEST;
-------------------
-- 정렬시 NULL
-- 오라클에서는 빈문자열 NULL값처리. 빈문자열 따로 NULL값 따로~ DB들마다 조금씩 다름
SELECT * FROM TEST ORDER BY STR2; -- 기본 오름차순 정렬, ASC 키워드 생략 가능
SELECT * FROM TEST ORDER BY STR2 DESC; -- DESC : 내림차순 (DESCENDING)

-- 정렬시 오라클에서는 NULL 값을 가장 큰 값으로 처리(맨 마지막 출력)
-- NULL 값 조회 순서 변경 : NULLS FIRST, NULLS LAST
SELECT * FROM TEST ORDER BY NUM;
SELECT * FROM TEST ORDER BY NUM DESC;
SELECT * FROM TEST ORDER BY NUM NULLS FIRST; -- NULL값을 맨 앞으로! NULL을 먼저!
SELECT * FROM TEST ORDER BY NUM DESC NULLS LAST; -- NULL값을 맨 뒤로!
-------------- 
INSERT INTO TEST (NUM, STR1, STR2) VALUES (200, '', NULL); -- '' : 빈칸
SELECT * FROM TEST WHERE STR1 = ''; -- 데이터 조회 안 됨 , 뜻 : 빈문자열이 있는 곳은 어디인가?
...
CREATE TABLE TEST2 (
    ...
    INNUM NUMBER(10,3), -- !주의! (전체자리,소수) // 정수부 7자리, 소수부 3자리 
    REGDATE DATE
);
SELECT * FROM TEST2;
INSERT INTO TEST2
...
ROLLBACK; -- 취소가능(DELETE 명령 사용시) 다시 살려냄. 오토커밋 상태가 아닌 경우에만 가능
    
-- TRUNCATE 명령문 : 테이블 전체 데이ㅓ 삭제(ROLLBACK으로 복구 안 됨)
