...
-- DDL문 사용 테이블 생성
CREATE TABLE MEMBER (            -- 최소한 하나 이상의 칼럼이 있어야 함
...
VALUES ('HONG', '홍길동', '1234'); // Java에서는 "홍길동" 쌍따옴표를 썼지만 DBL에서는 '홍길동' 따옴표 씀.

-- DCL : ROLLBACK, COMMIT
ROLLBACK; // rollback - 임시 처리된 작업 취소 처리. 롤백 처리하면 사라짐.
COMMIT; -- 임시 처리된 작업 최종 적용

-- 키 중복 데이터 입력시 : ORA-00001: unique constraint (MYSTUDY.SYS_C006999) violated
INSERT INTO MEMBER (ID, NAME, PASSWORD)
VALUES ('HONG2', '홍길동2', '1234'); // 데이터는 대소문자 구분 가능

-- NOT NULL 컬럼데이터 미입력 오류 
-- ORA-01400: cannot insert NULL into ("MYSTUDY"."MEMBER"."PASSWORD") <- . : ~에 있는 ~를 참조하는~                       
INSERT INTO MEMBER (ID, NAME)
VALUES ('HONG3', '홍길동3');
------------------------------
                      // * : ALL 전체를 의미 (* 읽을 때도 ALL로 읽으면 됨)
SELECT * FROM MEMBER; -- 테이블에 있는 모든 데이터 + 모든 컬럼 
SELECT ID, NAME, PASSWORD, PHONE FROM MEMBER; // 테이블 순서와 상관 없이 컬럼 작성한 순서대로 데이터를 가져옴.
---------                                     // 선택해서 입력한 값만 가져온다.
-- 입력 : 컬럼명 명시적으로 쓰지 않고 INSERT문 작성
-- 테이블 모든 컬럼 사용 + 컬럼 순서대로 데이터 입력
INSERT INTO MEMBER --> () 생략 :  되면 모든 컬럼을 다 한다는 뜻
VALUES ('HONG3', '홍길동3', '1234', '010-1111-1111', '서울');
COMMIT;

INSERT INTO MEMBER  
VALUES ('HONG4', '홍길동4', '1234', '서울시', '010-1111-1234');
COMMIT;

-------
-- 컬럼을 명시적으로 작성시 해당 컬럼에 1대 1 매칭되어 데이터 입력
...
UPDATE MEMBER SET PHONE = '010-1111-1234' WHERE ID = 'HONG4'; // ID가 HONG4하고 같은 거 찾기
UPDATE MEMBER SET ADDRESS = '서울시' WHERE ID = 'HONG4'; // 수정,삭제 작업할 때 반드시 select 써서 잘 찾아졌는지 확인
COMMIT;
----
...
------------------------------
-- 삭제 : HONG5 데이터 삭제
SELECT * FROM MEMBER WHERE ID = 'HONG5'; // 5만 찾음
DELETE FROM MEMBER WHERE ID = 'HONG5'; // DELETE FROM MEMBER; <- 전체 데이터 삭제
COMMIT;
-----
...
DELETE FROM MEMBER WHERE NAME = '홍길동'; // (주의) DELETE 할 때 조건 항상 확인!
COMMIT;
-- =====================
/* (실습) 입력, 수정, 삭제, 조회(검색)
■입력
...
UPDATE MEMBER SET ADDRESS = '서울시 강남구 역삼동' WHERE ID = 'HONG8';

SELECT * FROM MEMBER WHERE ID = 'HONG2'; // 2만 찾음
