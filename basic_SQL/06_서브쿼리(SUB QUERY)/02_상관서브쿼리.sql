...
SELECT *
 ...
 WHERE B.PUBLISHER = A.PUBLISHER -- 조인조건
   AND B.PRICE > A.AVG_PRICE -- 찾을(검색/선택)조건
;
--=============================================
SELECT *
  FROM CUSTOMER
...
SELECT * FROM CUSTOMER; -- 확인용
SELECT * FROM ORDERS; -- 확인용

SELECT * 
  FROM CUSTOMER C
 WHERE EXISTS (SELECT CUSTID FROM ORDERS -- EXISTS 를 쓰면 속도가 개선됨
               WHERE CUSTID = C.CUSTID)
;
--====> NOT EXIST 적용 : 데이터가 없으면 선택
SELECT * 
  FROM CUSTOMER C
 WHERE NOT EXISTS (SELECT 1 FROM ORDERS -- SELECT 절에 값은 2,3개 와도 상관없음
               WHERE CUSTID = C.CUSTID)
;