...
CREATE INDEX IX_BOOK_PUB ON BOOK (PUBLISHER); -- NON UNIQUE 이 컬럼값은 중복값이 있다.
...
SELECT * FROM BOOK WHERE PUBLISHER = '굿스포츠' AND PRICE > 7000; -- 인덱스 사용
SELECT * FROM BOOK WHERE PUBLISHER = '굿스포츠'; -- 인덱스 사용
SELECT * FROM BOOK WHERE PRICE >= 20000; -- 인덱스 사용X(안 됨)
SELECT * FROM BOOK WHERE PRICE >= 7000 AND PUBLISHER = '굿스포츠'; -- 순서 바뀌어도 상관없이 인덱스 사용
--------
-- (주의) 인덱스가 있음에도 적용되지 않는 경우
---- 1) LIKE 사용시 OR 2) 데이터가 가공된 경우
SELECT * FROM BOOK WHERE PUBLISHER LIKE '대한%'; -- 인덱스 사용
SELECT * FROM BOOK WHERE PUBLISHER LIKE '%미디어'; -- 인덱스 사용못함(안 함)
SELECT * FROM BOOK WHERE PUBLISHER LIKE '%대한%'; -- 인덱스 적용 안 함

-- 원본 데이터를 가공(변형)하는 경우(인덱스 적용 안 됨)
SELECT * FROM BOOK WHERE PUBLISHER = INITCAP ('pearson'); -- 이건 되지롱!
SELECT * FROM BOOK WHERE UPPER(PUBLISHER) = 'PEARSON'; -- 적용못함
SELECT * FROM BOOK WHERE SUBSTR(PUBLISHER, 1, 2) = '대한'; -- 적용못함. 컬럼값을 가공하면 적용이 안 된다.
...
--(3) Customer 테이블에 name으로 인덱스(ix_customer_name)를 생성하시오. 
--    생성 후 (1)번의 질의를 다시 수행하고 실행 계획을 살펴보시오.
--(4) 같은 질의에 대한 두 가지 실행 계획을 비교해보시오.
