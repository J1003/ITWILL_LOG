...
SELECT SYSDATE, SYSDATE +1, SYSDATE -1 FROM DUAL;  -- "내일, 어제 이 시간" 출력
...
-- NEXT_DAY(날짜, 요일) : 요일 날짜 구하기, 날짜 다음에 오는 첫번째 요일날짜(일) 구하기
SELECT SYSDATE, NEXT_DAY(SYSDATE, '화') FROM DUAL; -- 15일 (8월9일 기준)
SELECT SYSDATE, NEXT_DAY(SYSDATE, '수') FROM DUAL; -- 16일
SELECT SYSDATE, NEXT_DAY(SYSDATE, '토요일') FROM DUAL; -- 12일

-- MONTHS_BETWEEN(날짜1, 날짜2) : 기간 구하기(개월) 연산방식(날짜1 - 날짜2)
SELECT SYSDATE, ADD_MONTHS(SYSDATE, 2) FROM DUAL; -- 오늘날짜부터 2개월 후
SELECT SYSDATE, ADD_MONTHS(SYSDATE+1, 2) FROM DUAL; -- 내일부터 2개월 후
SELECT SYSDATE, ADD_MONTHS(SYSDATE-1, 2) FROM DUAL; -- 어제부터 2개월 후

SELECT MONTHS_BETWEEN(SYSDATE, ADD_MONTHS(SYSDATE, 2)) FROM DUAL; --> -2 출력
SELECT MONTHS_BETWEEN(ADD_MONTHS(SYSDATE, 2), SYSDATE) FROM DUAL; --> 2 출력
--============================
-- 1 : 1일 -> 24시간, 1시간 : 1일 / 24시간 --> 1시간
-- 1시간 : 60분 ---> 1시간 / 60분 ---> 1분 :::: 1 / 24 / 60 --> 1분
-- 1분 : 60초 ---> 1분 / 60초 -> 1초 :::: 1 / 24 / 60 / 60 --> 1초
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI:SS')   -- TO_CHAR -- 문자로
    , TO_CHAR(SYSDATE + 1, 'YYYY-MM-DD HH24:MI:SS') -- 하루 뒤
...
-- 1일 : 86400초, 1440분
SELECT 1 *60 AS "1분(초)",  -- 숫자랑 문자랑 같이 쓸 때 OR "ONE MIN" 이럴 때 "" 쌍따옴표
       60*60 AS "1시간(초)", 
       24*60*60 AS "1일(초)", 
       24*60 AS "1일(분)"
FROM DUAL;
