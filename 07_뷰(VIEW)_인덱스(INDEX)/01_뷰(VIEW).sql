...
---------------------
-- 뷰 삭제(DROP)
DROP VIEW VW_BOOK; -- 뷰 삭제 / 다시 생성하려면 CREATE 하면 됨
---------------------
...
SELECT BOOKNAME, PRICE, ORDERDATE, PUBLISHER 
  FROM VW_ORD_ALL 
...
--4. 판매된 책에 대하여 구매자별 건수, 합계금액, 평균금액, 최고가, 최저가 조회
--(최종) 뷰삭제 : VW_ORD_ALL 삭제 처리
SELECT COUNT(*), SUM(SALEPRICE), TRUNC(AVG(SALEPRICE)), MAX(SALEPRICE), MIN(SALEPRICE)
  FROM VW_ORD_ALL
 GROUP BY CUSTID
;
